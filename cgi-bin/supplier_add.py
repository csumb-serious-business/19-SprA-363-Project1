...
class SupplierAdd(BaseCGI):
    ...
    q_supplier_add = '''
        INSERT INTO supplier (name, address, phone) VALUES ('%s', '%s', '%s');
        '''
    ...
    def __init__(self):
        super().__init__('./html-templates/supplier_add.html')

    def _run(self):
        form = cgi.FieldStorage()
        # todo + update supplier details

        # retrieve input values
        name = form["name"].value
        address = form["address"].value
        phone = form["phone"].value
        phone = re.sub(r'\D', '', phone)  # remove all non-digit chars

        exists = self.query_get_first(self.q_supplier_exists % name)
        if not exists:
            self.query_committed(self.q_supplier_add % (name, address, phone))
            self.add_result('result', 'Added Supplier: %s' % name)
        else:
            self.add_result('result', '%s is already in Suppliers' % name)

        updated_items = [('name', 'address', 'phone')]
        updated_items.extend(self.query_get_all(self.q_supplier))
